...
static ros::Subscriber subSkilledWorkcellStepDone;
static ros::Subscriber subPerceptionStepDone;
static ros::Subscriber subDeformationControlStepDone;
...
static void perceptionStepDoneCallback(const std_msgs::Bool::ConstPtr& msg);
static void deformationControlStepDoneCallback(const std_msgs::Bool::ConstPtr& msg);


bool isSkilledWorkcellDone = false;
bool isPerceptionDone = false;
bool isDeformationControlDone = false;
...
softmanbotState state_machine(void)
{
	...
	//TEST
	//Rustine 
	//if(previousReadState == PERFORMT1S3)
	//{
	//	return PERFORMT1S3;
	//}// !TEST
	...
	softmanbot_ros::PLC_WriteTag srvWriteStep;		
	srvWriteStep.request.tagName = softmanbotStatusStepTag;
	srvWriteStep.request.valueToWrite = WORK_IN_PROGRESS;
	...
	case 0:
		if(IDLE != previousReadState)
		{
			...
			//clientPlcWrite.call(srvWriteStep); //No step to step in idle
	...
	case 1:
		...
		{
			clientPlcWrite.call(srvWrite);
			clientPlcWrite.call(srvWriteStep);
...
{
	ros::NodeHandle nh;
	subSkilledWorkcellStepDone = nh.subscribe("skilledWorkcellStepDone", 1, skilledWorkcellStepDoneCallback);
	subPerceptionStepDone = nh.subscribe("perceptionStepDone", 1, perceptionStepDoneCallback);
	subDeformationControlStepDone = nh.subscribe("deformationControlStepDone", 1, deformationControlStepDoneCallback);
...
void michelinSupervisoryInterface::supervisoryInit(void)
{
	...
	if(isSkilledWorkcellDone && isPerceptionDone)
	{
		ROS_INFO("Notifying PLC we're done");
...
void michelinSupervisoryInterface::supervisoryPerformT1S3(void)
{
	...
	if(isDeformationControlDone)
	{
		isDeformationControlDone = false;
...
static void skilledWorkcellStepDoneCallback(const std_msgs::Bool::ConstPtr& msg)
{
	ROS_INFO("SkillWorkcellStepDone Callback");
...
static void deformationControlStepDoneCallback(const std_msgs::Bool::ConstPtr& msg)
{
	ROS_INFO("deformationControlStepDone Callback");
	isDeformationControlDone = true;
}
